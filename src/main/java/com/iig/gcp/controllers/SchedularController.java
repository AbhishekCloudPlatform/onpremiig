...
public class SchedularController {
	...
	/**
	 * This method populated the View Run Statics Screen with list of Job Id w.r.t. feedid
	 * @param feed_id
	 * @param modelMap
	 * @return
	 */
	...
	public ModelAndView getAreachart(@Valid @RequestParam("job_id")String job_id,@Valid @RequestParam("feed_id")String feed_id,ModelMap modelMap) {
	...
	public ModelAndView frequencySelect(@Valid @RequestParam("frequency") String frequency,@RequestParam("batchId") String batchId, ModelMap modelMap) {
		try {
	...
	public ModelAndView batchIdSelect(@Valid  @RequestParam("frequency") String frequency,@RequestParam("batchId") String batchId, ModelMap modelMap) {
		try {
			List<MasterJobsDTO> dtos = schedularService.typeLoadJobs(frequency,batchId);
			modelMap.addAttribute("allLoadJobs", dtos);
		}catch(Exception e) {
	...
	public ModelAndView statusFilter(@Valid @RequestParam("status") String status,@RequestParam("feedid") String feedid, ModelMap modelMap) {
		try {
	...
	public ModelAndView feedFilter(@Valid @RequestParam("status") String status,@RequestParam("feedid") String feedid, ModelMap modelMap) {
		try {
	...
	/*
	@RequestMapping(value = { "/scheduler/scheduledjobs"}, method = RequestMethod.GET)
	public ModelAndView scheduledJobs(ModelMap modelMap) {
		try {	
			HashMap<String,List<DailyJobsDTO>> hsMap = schedularService.allCurrentJobs();
			//HashMap<String, ArrayList<String>> hsMapCount = schedularService.allCurrentJobsGroupByFeedId();
		
		System.out.println("Key"+hsMapCount.get("arrkey"));
		
		System.out.println("value"+hsMapCount.get("arrValue"));
			int totalCount=hsMap.get("completed").size()+hsMap.get("failed").size()+ hsMap.get("notstarted").size();
		...
		return new ModelAndView("schedular/viewScheduledjobs");
	}*/
